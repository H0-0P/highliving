...
<table class="easyui-datagrid" id="itemParamList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'http://192.168.8.2:8080/highliving/item/param/list',method:'get',pageSize:30,toolbar:itemParamListToolbar">
    <thead>
        <tr>
        	<th data-options="field:'ck',checkbox:true"></th>
        	<th data-options="field:'modelid',width:60">ID</th>
        	<th data-options="field:'goodtypeid',width:80">商品类目ID</th>
        	<th data-options="field:'goodname',width:100">商品类目</th>
...
<script>
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	TAOTAO.createWindow({
        		url : "http://192.168.8.2:8080/highliving/item-param-add",
        	});
        }
    },{
        text:'编辑',
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品规格吗？',function(r){
        	    if (r){
        	    	var params = "params=[{'ids':"+ids+"}]";
                	$.post("http://192.168.8.2:8080/highliving/item/param/delete",params, function(data){
            			if(data.status == 200){
